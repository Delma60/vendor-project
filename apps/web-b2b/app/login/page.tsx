...
export default function LoginPage() {
	...
	return <main className="public-shell"><section className="public-content"><Card title="Company portal sign in" description="Use your FoodConnect Business account to continue."><form className="auth-form" onSubmit={handleSubmit}><Field label="Email" htmlFor="email"><Input id="email" type="email" value={email} onChange={event => setEmail(event.target.value)} required /></Field><Field label="Password" htmlFor="password" error={error ?? undefined}><Input id="password" type="password" value={password} onChange={event => setPassword(event.target.value)} required /></Field><div className="topbar-actions"><Button type="submit" disabled={submitting}>{submitting ? 'Signing in...' : 'Sign in'}</Button><Link className="btn btn-ghost" href="/forgot-password">Forgot password?</Link></div></form></Card></section></main>;
}
