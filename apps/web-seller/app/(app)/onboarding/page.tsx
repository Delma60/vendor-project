...
import { Button, Card, Field, FileUpload, Input, Select, Stepper, Textarea, useToast } from '@foodconnect/ui/components';
...
export default function SellerOnboardingPage() {
  ...
  const handleDocumentSelected = (label: string) => (file: File | null) => setForm(current => ({ ...current, documents: [...current.documents.filter(document => document.label !== label), ...(file ? [{ id: crypto.randomUUID(), label, fileName: file.name, uploadedAt: new Date().toISOString() }] : [])] }));
  const handleSubmit = () => { setSubmitted(true); toast({ title: 'Application submitted', message: 'Your seller profile is pending admin approval.', tone: 'success' }); };
  const goNext = () => (isLastStep ? handleSubmit() : setStepIndex(index => Math.min(index + 1, STEPS.length - 1)));
  const goBack = () => setStepIndex(index => Math.max(index - 1, 0));

  if (submitted) return <Card title="Application submitted" description="Thanks for completing your seller profile."><div className="pending-approval"><span className="pending-approval-icon" aria-hidden="true">⏳</span><h2>Your account is pending approval</h2><p className="muted">Our team is reviewing {form.businessName || 'your business'}&apos;s documents and details. This usually takes 1-2 business days. We&apos;ll email you once you&apos;re approved.</p><Button variant="secondary" onClick={() => { setSubmitted(false); setStepIndex(0); }}>Edit application</Button></div></Card>;
  ...
  return <Card title="Business onboarding" description="Complete each step to submit your seller profile for approval.">
    ...
    <div className="onboarding-actions"><Button variant="ghost" onClick={goBack} disabled={stepIndex === 0}>Back</Button><Button onClick={goNext}>{isLastStep ? 'Submit for approval' : 'Continue'}</Button></div>
  </Card>;
}
